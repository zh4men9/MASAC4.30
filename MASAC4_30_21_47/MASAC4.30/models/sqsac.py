...
class SQSAC(Model):
    ...
    def __init__(self, args, target_net=None):
        super(SQSAC, self).__init__(args)
    ...
    def get_loss(self, batch, alphas):  # batch是一个包含了状态、动作、奖励等信息的数据集
        ...
        advantages = shapley_values-alphas * log_pi_a
        if self.args.normalize_advantages:
            advantages = batchnorm(advantages)
        action_loss = -advantages
        action_loss = action_loss.mean(dim=0)
        # do torche exploration action on torche value loss
        shapley_values_sum = self.marginal_contribution(state, actions).mean(dim=1).contiguous().view(-1, n).sum(dim=-1,
                                                                                                                 keepdim=True).expand(
            batch_size, self.n_)
        ...
        if self.args.target:
            next_shapley_values_sum = self.target_net.marginal_contribution(next_state, next_actions_.detach()).mean(
                dim=1).contiguous().view(-1, n).sum(dim=-1, keepdim=True).expand(batch_size, self.n_)
        else:
            next_shapley_values_sum = self.marginal_contribution(next_state, next_actions_.detach()).mean(
                dim=1).contiguous().view(-1, n).sum(dim=-1, keepdim=True).expand(batch_size, self.n_)
        ...
        for i in reversed(range(rewards.size(0))):
            if last_step[i]:
                next_return1 = 0 if done[i] else next_shapley_values_sum[i].detach()
                next_return2 = 0 if done[i] else log_pi_a_next[i].detach()
            else:
                next_return1 = next_shapley_values_sum[i].detach()
                next_return2 = 0 if done[i] else log_pi_a_next[i].detach()
