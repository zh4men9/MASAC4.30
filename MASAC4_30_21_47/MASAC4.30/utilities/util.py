...
def select_act(args, logits, status='train', info={}):
    act_mean = logits  # 输出动作均值
    act_std = cuda_wrapper(torch.ones_like(act_mean), args.cuda)  # 新建标准差为1的tensor。cuda_wrapper函数会把tensor移到GPU上
    if status is 'train':
        action_dist = Normal(act_mean, act_std)
        action_out = action_dist.sample()
        return action_dist, action_out  # 如果处于训练状态，根据均值act_mean和标准差act_std从正态分布中采样一个动作
    elif status is 'test':
        return act_mean
